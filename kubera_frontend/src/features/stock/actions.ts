...
export async function buyStock(formData: FormData): Promise<StockActionResult> {
  ...
  try {
    await apiFetch("/inventory/batches/quick", { method: "POST", body: JSON.stringify({ fruit_id: text("fruit_id"), fruit_name: text("fruit_name"), supplier_id: text("supplier_id"), supplier_name: text("supplier_name"), mark: text("mark"), phone: text("phone"), location_label: text("location_label"), latitude: text("latitude") ? number("latitude") : null, longitude: text("longitude") ? number("longitude") : null, quality: text("quality") || null, size: text("size"), quantity: number("quantity"), unit: text("unit"), purchase_price_per_unit: number("purchase_price_per_unit") }) });
