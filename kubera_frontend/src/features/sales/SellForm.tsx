...
export function SellForm({ locale, currency = "INR", timezone = "UTC", batches }: { locale: string; currency?: string; timezone?: string; batches: Batch[] }) {
  ...
  const effectiveCost = selected?.purchase_price_per_unit ?? (price === "" ? null : Number(price) * 0.94);
  const expectedProfit = effectiveCost == null ? null : Number(quantity) * (Number(price) - effectiveCost);
  ...
  return <form onSubmit={submit} className="space-y-5">
    ...
    <div className="rounded-2xl border border-[#dce6df] bg-[#f7faf8] px-4 py-3"><p className="text-xs font-bold uppercase tracking-wider text-[#648072]">{selected?.purchase_price_per_unit == null ? t("suggestedBuy") : t("boughtFor")}</p><p className="mt-1 text-lg font-bold text-[#173f31]">{effectiveCost == null ? t("buyPriceUnknown") : `${formatMoney(effectiveCost, locale, currency)} / ${selected?.unit ?? ""}`}</p>{selected?.purchase_price_per_unit == null && <p className="mt-1 text-xs text-[#747970]">{t("buyPriceUnknown")}</p>}</div>
