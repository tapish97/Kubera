...
import { DailyReportPreview, type DetailedDailyReport } from "@/features/reports/DailyReportPreview";
...
export default async function ShopPage({params,searchParams}:{params:Promise<{locale:string}>;searchParams:Promise<{tab?:string;date?:string;generate?:string}>}) {
  const {locale}=await params; const query=await searchParams; const t=await getTranslations({locale,namespace:"Shop"}); const closingT=await getTranslations({locale,namespace:"Closing"});
  ...
  const report=tab==="reports"&&query.generate==="1"?await apiFetch<DetailedDailyReport>(`/reports/daily?date=${encodeURIComponent(date)}`):null;
  ...
  return <AppShell><div className="px-5 pt-[max(1.25rem,env(safe-area-inset-top))]">
    ...
    {tab==="overview"&&<section className="mt-5 space-y-3"><ActionLink href={`/${locale}/closing`} title={closingT("title")} hint={closingT("hint")}/><ActionLink href={`/${locale}/settlements`} title={t("buyingPrices")} hint={t("buyingPricesHint")} count={notification.unpriced_batch_count}/><ActionLink href={`/${locale}/suppliers`} title={t("marks")} hint={t("marksHint")}/><Link href={`/${locale}/shop?tab=reports`} className="flex min-h-16 items-center justify-between rounded-2xl border border-[#e7e1d5] bg-white px-4"><span><b>{t("dailyReports")}</b><small className="mt-1 block text-[#747970]">{t("dailyReportsHint")}</small></span><span>→</span></Link></section>}
    {tab==="reports"&&<section className="mt-5"><form className="flex items-end gap-2 rounded-2xl border border-[#e7e1d5] bg-white p-4 print:hidden"><input type="hidden" name="tab" value="reports"/><input type="hidden" name="generate" value="1"/><label className="min-w-0 flex-1 text-sm font-bold">{t("reportDate")}<input name="date" type="date" defaultValue={date} max={todayIn(account.timezone)} className="mt-2 min-h-12 w-full rounded-xl border border-[#ded9cf] px-3"/></label><button className="min-h-12 rounded-xl bg-[#216148] px-4 font-bold text-white">{t("generate")}</button></form>{!report&&<p className="mt-5 text-center text-sm text-[#747970]">{t("chooseDay")}</p>}{report&&<>{report.unpriced_batch_count>0&&<div className="mt-3 rounded-2xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950 print:hidden"><b>{t("missingPrices",{count:report.unpriced_batch_count})}</b><p className="mt-1">{t("estimatedExplanation")}</p><Link href={`/${locale}/settlements`} className="mt-3 inline-block font-bold underline">{t("addPrices")}</Link></div>}<DailyReportPreview report={report} shopName={account.shop_name} locale={locale} currency={account.currency}/></>}</section>}
